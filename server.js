...
const db = new sqlite3.Database('ESP32.db', err => {
  if (err) {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS data(
      ...
      if (err) {
        console.error('Erreur lors de la création de la table data :', err.message);
      } else {
...
function saveData(data) {
  ...
  db.get(selectVQuery, (err, row) => {
    if (err) {
      console.error('Erreur lors de la vérification de l\'existence des données :', err.message);
    } else {
      if (row) {
        ...
        db.run(insertQuery, receivedData, nameData, function (err) {
          if (err) {
            ...
            console.log('Données enregistrées dans la base de données avec l\'ID :', this.lastID);
          }
        });
      }
    }
